...
const PlantDetails: Component<Props> = (props) => {
  const plantedAt = createMemo(() => props.plant.plantedAt && new Intl.DateTimeFormat('fr', {dateStyle: 'long', timeStyle: 'short'}).format(new Date(props.plant.plantedAt)));

  return (<Stack>
    {props.plant.plantedAt && <Typography variant="body2">Planté le <Typography variant="subtitle2" component="span">{plantedAt()}</Typography></Typography>}
