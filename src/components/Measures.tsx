...
import type { MeasureGraph } from '@/models/measure-graph';
...
const measureGraphToFeatureCollection = (
  graph: MeasureGraph,
): FeatureCollection => ({
...
const Measures: Component<Props> = (props) => {
  const measureFeatureCollection = createMemo(() =>
    measureGraphToFeatureCollection(props.graph),
  );
  const map = useMap();

  onCleanup(() => {
    const delegateListeners = map()?._delegatedListeners.click.filter(
      (l) => l.layer === measureLayerId,
    );
  ...
  return (
    <Source source={{ type: 'geojson', data: measureFeatureCollection() }}>
      <Layer
        style={{
          type: 'line',
          paint: {
      ...
      <Layer
        ...
        onClick={(evt: MapLayerMouseEvent) => {
          ...
          clickedMeasureFeature &&
            props.onMeasureClick([
              clickedMeasureFeature.properties.aId,
              clickedMeasureFeature.properties.bId,
            ]);
        }}
      />
    </Source>
